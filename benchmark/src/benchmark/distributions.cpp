...
namespace Distributions {
    ...
    random_duplicates(std::uint64_t size, const Settings &settings, std::uint32_t p, std::uint64_t range) {
        ...
        for (std::uint64_t i = 0; i < p; ++i) {
            ...
            std::generate(regions.begin(), regions.end(),
                          [&]() { return random() % static_cast<std::uint32_t>(range); });
    ...
    template Distribution<std::uint32_t>
    create(Type::Value type, std::uint64_t size, const Settings &settings, std::uint32_t p, std::uint32_t g,
           std::uint32_t range);

    template Distribution<std::uint64_t>
    create(Type::Value type, std::uint64_t size, const Settings &settings, std::uint32_t p, std::uint32_t g,
           std::uint32_t range);

    template Distribution<std::uint16_t> zero(std::uint64_t size);

    template Distribution<std::uint16_t> sorted(std::uint64_t size, const Settings &settings);

    template Distribution<std::uint16_t> sorted_descending(std::uint64_t size, const Settings &settings);

    template Distribution<std::uint16_t> uniform(std::uint64_t size, const Settings &settings);

    template Distribution<std::uint16_t> gaussian(std::uint64_t size, const Settings &settings);

    template Distribution<std::uint16_t> bucket(std::uint64_t size, const Settings &settings, std::uint32_t p);

    template Distribution<std::uint16_t> staggered(std::uint64_t size, const Settings &settings, std::uint32_t p);

    template Distribution<std::uint16_t>
    g_groups(std::uint64_t size, const Settings &settings, std::uint32_t p, std::uint32_t g);

    template Distribution<std::uint16_t>
    random_duplicates(std::uint64_t size, const Settings &settings, std::uint32_t p, std::uint64_t range);

    template Distribution<std::uint16_t>
    deterministic_duplicates(std::uint64_t size, const Settings &settings, std::uint32_t p);

    template Distribution<std::uint32_t> zero(std::uint64_t size);

    template Distribution<std::uint32_t> sorted(std::uint64_t size, const Settings &settings);

    template Distribution<std::uint32_t> sorted_descending(std::uint64_t size, const Settings &settings);

    template Distribution<std::uint32_t> uniform(std::uint64_t size, const Settings &settings);

    template Distribution<std::uint32_t> gaussian(std::uint64_t size, const Settings &settings);

    template Distribution<std::uint32_t> bucket(std::uint64_t size, const Settings &settings, std::uint32_t p);

    template Distribution<std::uint32_t> staggered(std::uint64_t size, const Settings &settings, std::uint32_t p);
    ...
    template Distribution<std::uint64_t> zero(std::uint64_t size);

    template Distribution<std::uint64_t> sorted(std::uint64_t size, const Settings &settings);

    template Distribution<std::uint64_t> sorted_descending(std::uint64_t size, const Settings &settings);

    template Distribution<std::uint64_t> uniform(std::uint64_t size, const Settings &settings);

    template Distribution<std::uint64_t> gaussian(std::uint64_t size, const Settings &settings);

    template Distribution<std::uint64_t> bucket(std::uint64_t size, const Settings &settings, std::uint32_t p);

    template Distribution<std::uint64_t> staggered(std::uint64_t size, const Settings &settings, std::uint32_t p);
