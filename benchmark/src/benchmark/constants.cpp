...
namespace Benchmark {
    Settings Settings::parse_from_cmd(int argc, const char *argv[], bool disable_exception_handling) {
        ...
        TCLAP::ValueArg<std::string> algorithm("a", "algorithm",
                                               "Algorithm to benchmark.", true, "samplesort",
        ...
        TCLAP::ValueArg<std::uint32_t> parameter_p("p", "parameter-p",
                                                   "Parameter P for the distribution types bucket, g-groups, staggered, random-duplicates, deterministic-duplicates.",
                                                   false, 128, "int");
        ...
        TCLAP::ValueArg<std::uint32_t> parameter_range("r", "parameter-range",
                                                       "Parameter Range for the distribution type random-duplicates.",
                                                       false, 30, "int");
        ...
        TCLAP::ValueArg<std::size_t> samples_per_key("s", "samples",
                                                     "Random samples taken per key. The samples are combined with logical and; taking more samples reduces the entropy of the data.",
                                                     false, 1, "int");
        TCLAP::ValueArg<std::size_t> bits_per_key("b", "bits",
                                                  "Number of bits per key that are not zeroed out. Choosing a lower bit count reduces the entropy of the data.",
                                                  false, 32, "int");
        TCLAP::SwitchArg keys_have_values("v", "keys-have-values",
                                          "If set, a key-value sort is done, where the values are 64bit integers.",
                                          false);
        TCLAP::ValueArg<std::string> output("o", "output-file",
                                            "Destination file for the CSV output of the benchmark run.", false,
        ...
        return {Algorithm::parse(algorithm.getValue()),
