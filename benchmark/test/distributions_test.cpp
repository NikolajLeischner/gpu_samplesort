...
namespace {
	...
	void assert_common_properties(Distribution<std::uint32_t> distribution, Distribution<std::uint32_t> other, std::size_t expected_size) {
...
go_bandit([](){
  describe("Random distributions", [&]() {
	  ...
	  describe("uniform random distributions", [&]() {
		  assert_common_properties(uniform<std::uint32_t>(size, settings), uniform<std::uint32_t>(size, settings), size);
	  });

	  describe("bucket distributions", [&]() {
		  assert_common_properties(bucket<std::uint32_t>(size, settings, p), bucket<std::uint32_t>(size, settings, p), size);
	  });

	  describe("staggered distributions", [&]() {
		  assert_common_properties(staggered<std::uint32_t>(size, settings, p), staggered<std::uint32_t>(size, settings, p), size);
	  });

	  describe("g-groups distributions", [&]() {
		  std::uint32_t g(3);
		  assert_common_properties(g_groups<std::uint32_t>(size, settings, p, g), g_groups<std::uint32_t>(size, settings, p, g), size);
	  });

	  describe("random duplicates distributions", [&]() {
		  std::size_t range(100);
		  assert_common_properties(random_duplicates<std::uint32_t>(size, settings, p, range), random_duplicates<std::uint32_t>(size, settings, p, range), size);
	  });

	  describe("deterministic duplicates distributions", [&]() {
		  std::size_t range(100);
		  assert_common_properties(deterministic_duplicates<std::uint32_t>(size, settings, p), deterministic_duplicates<std::uint32_t>(size, settings, p), size);
	  });
  });
});